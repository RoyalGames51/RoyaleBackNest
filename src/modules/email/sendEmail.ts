...
export async function sendEmail({ nick, email, option }) {
  console.log(nick, email, option);
  ...
  try {
    ...
    switch (option) {
      case 'signIn':
        preSubject = `¡Hola ${userName.toUpperCase(
          0,
        )}! Bienvenida/o a RoyalGames!!`;
    ...
    const confirmEmail = await mailing(userName, email, preSubject, message);
    if (confirmEmail) return { message: 'Email sent successfully' };
  } catch (error) {
